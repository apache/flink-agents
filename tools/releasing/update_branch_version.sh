...
# Update jar_manifest.json version (strip -SNAPSHOT suffix for Maven Central)
MANIFEST_VERSION=$(echo "${NEW_VERSION}" | sed 's/-SNAPSHOT//')
perl -pi -e "s#\"version\": \".*\"#\"version\": \"${MANIFEST_VERSION}\"#" jar_manifest.json
cd ..
