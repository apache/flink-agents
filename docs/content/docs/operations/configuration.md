...
| `short-term-memory.state-ttl.ms` | 0                   | long                  | Time-to-live for short-term memory state, in milliseconds. Set to a positive value to enable TTL; `0` (the default) disables it. The three options below are consulted only when TTL is enabled. |
| `short-term-memory.state-ttl.update-type` | `ON_READ_AND_WRITE` | ShortTermMemoryTtlUpdate | When the TTL timer is reset. Valid values: `ON_CREATE_AND_WRITE` (reset on create and write) and `ON_READ_AND_WRITE` (additionally reset on read). |
| `short-term-memory.state-ttl.visibility` | `NEVER_RETURN_EXPIRED` | ShortTermMemoryTtlVisibility | Whether expired state that has not yet been cleaned up can be read. Valid values: `NEVER_RETURN_EXPIRED` and `RETURN_EXPIRED_IF_NOT_CLEANED_UP`. |
| `short-term-memory.state-ttl.cleanup-strategy` | `FULL_SNAPSHOT` | ShortTermMemoryTtlCleanupStrategy | How expired state is reclaimed. Valid values: `FULL_SNAPSHOT` (during full checkpoint snapshots; the default), `INCREMENTAL` (incrementally on state access), `ROCKSDB_COMPACTION_FILTER` (during RocksDB background compaction; only effective with the RocksDB state backend), and `LAZY` (on read access only, disabling background cleanup). |
