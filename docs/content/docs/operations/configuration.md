...
| `tool-call.parallel`      | true                       | boolean               | When `tool-call.async` is also true, run multiple tool calls from one `ToolRequestEvent` as one durable batch. On **Java**, concurrent in-batch execution requires **JDK 21+** (Continuation API); below JDK 21 the batch still runs but tool calls execute serially. **Python** uses a dedicated `ThreadPoolExecutor` and runs batches concurrently regardless of JDK version. Increases in-flight external calls; after failover, unfinished tools may be submitted again — side-effecting tools should be idempotent or provide a reconciler. Set to `false` for serial tool execution. |
