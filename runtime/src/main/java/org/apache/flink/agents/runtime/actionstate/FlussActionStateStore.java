 ...
 *   <li><b>Recovery</b>: Scan log from recovery markers (or from beginning if none) to rebuild
 *       in-memory state.
...
public class FlussActionStateStore implements ActionStateStore {
     ...
     * Otherwise, subscribes from the beginning. Reads until the log is exhausted ({@link
     * #EMPTY_POLL_THRESHOLD} consecutive empty polls). For the same state key appearing multiple
     * times in the log, the latest record wins (last-write-wins).
     *
     * <p>Note: Unlike the Kafka implementation which skips rebuild on empty markers, this
     * implementation scans from the beginning when no markers are provided, enabling full state
    ...
    public void rebuildState(List<Object> recoveryMarkers) {
        ...
        for (Object marker : recoveryMarkers) {
            if (marker instanceof Map) {
                ...
                for (Map.Entry<Integer, Long> entry : markerMap.entrySet()) {
                    bucketOffsets.merge(entry.getKey(), entry.getValue(), Math::min);
                }
            } else if (marker != null) {
                LOG.warn(
        ...
        try (LogScanner scanner = table.newScan().createLogScanner()) {
            if (bucketOffsets.isEmpty()) {
                ...
                for (Map.Entry<Integer, Long> entry : bucketOffsets.entrySet()) {
    ...
    public Object getRecoveryMarker() {
        ...
        try (Admin admin = connection.getAdmin()) {
            ...
            LOG.error("Failed to get end offsets for Fluss table: {}", tablePath, e);
            throw new RuntimeException("Failed to get end offsets for Fluss table", e);
        }
    }
