...
public class TaskLifecycleListenerNotificationTest {
    ...
    /**
     * Listener that captures the durable action-state picture observed at {@code onTaskFinished}
     * time, so the test can assert the notification arrives before the completed state is
     * persisted.
     */
    private static class CompletionStateObservingListener implements TaskLifecycleListener {
        ...
        private volatile Integer stateCountAtFinished = null;
        ...
        public void onTaskFinished(ActionTask task) {
            List<ActionState> states =
                    store.getKeyedActionStates().values().stream()
                            .flatMap(perKey -> perKey.values().stream())
                            .collect(java.util.stream.Collectors.toList());
            stateCountAtFinished = states.size();
            anyStateCompletedAtFinished = states.stream().anyMatch(ActionState::isCompleted);
            events.add("finished:" + task.getAction().getName());
        }
    }

    @Test
    void taskFinishedIsNotifiedBeforeCompletedStateIsPersisted() throws Exception {
        AgentPlan plan = new AgentPlan(new SyncAgent());
        ...
        try (KeyedOneInputStreamOperatorTestHarness<Long, Long, Object> testHarness =
            ...
            assertThat(listener.events).containsExactly("finished:handleInput");
            // The action state was initialized before the invocation, but the finished
            ...
            assertThat(listener.stateCountAtFinished).isEqualTo(1);
            assertThat(listener.anyStateCompletedAtFinished).isFalse();
    ...
    @Test
    void replayOfCompletedActionEmitsPreparedAndFinishedPair() throws Exception {
        AgentPlan plan = new AgentPlan(new SyncAgent());
        InMemoryActionStateStore actionStateStore = new InMemoryActionStateStore(false);
        ...
        // skipped, but the prepared/finished pair must still be emitted.
        try (KeyedOneInputStreamOperatorTestHarness<Long, Long, Object> testHarness =
                new KeyedOneInputStreamOperatorTestHarness<>(
                        new TestableOperatorFactory(plan, actionStateStore),
                        (KeySelector<Long, Long>) value -> value,
