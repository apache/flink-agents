...
public class MockAsyncSubagentSetup extends BaseAsyncSubagentSetup {
    ...
    @Override
    protected Result callFetchResult(String sessionId, String callId) {
        fetchCount++;
        Run run = runs.get(sessionId + "#" + callId);
        if (run == null) {
            return Result.error("no run on record");
        }
        return run.error == null ? Result.ok(run.result) : Result.error(run.error);
    }
    ...
    public DurableCallable<Result> awaitResultForTest(String sessionId, String callId) {
        return awaitResult(null, sessionId, callId);
    }

    /** Exposes the fetch durable call for unit-style assertions. */
    public DurableCallable<Result> fetchResultForTest(String sessionId, String callId) {
        return fetchResult(null, sessionId, callId);
    }
}
