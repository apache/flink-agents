...
The Java operator forwards its per-record and per-task lifecycle to Python over
pemja, and ``FlinkRunnerContext`` fans each callback out to the registered
``TaskLifecycleListener``s. These tests exercise that registration and fan-out
with a recording listener, without a live interpreter.
"""
...
class _RecordingListener(TaskLifecycleListener):
    ...
    def on_task_prepared(self, task: Any) -> None:
        self.calls.append(("task_prepared", task))

    def on_task_transferred(self, from_task: Any, to_task: Any) -> None:
        self.calls.append(("task_transferred", from_task, to_task))

    def on_task_finished(self, task: Any) -> None:
        self.calls.append(("task_finished", task))
...
def test_fan_out_forwards_every_callback_in_order() -> None:
    """Each of the five callbacks reaches the registered listener, in order."""
    ctx = _context()
    listener = _RecordingListener()
    ctx.add_task_lifecycle_listener(listener)

    ctx.notify_record_start("k")
    ctx.notify_task_prepared("t")
    ctx.notify_task_transferred("t", "t2")
    ctx.notify_task_finished("t2")
    ctx.notify_record_finished("k")

    assert listener.calls == [
        ("record_start", "k"),
        ("task_prepared", "t"),
        ("task_transferred", "t", "t2"),
        ("task_finished", "t2"),
        ("record_finished", "k"),
    ]
...
def test_fan_out_reaches_every_registered_listener() -> None:
    ...
    assert first.calls == [("task_prepared", "t")]
    assert second.calls == [("task_prepared", "t")]
...
def test_module_entries_delegate_to_the_context() -> None:
    ...
    frc.notify_task_prepared(ctx, "t")
    frc.notify_task_transferred(ctx, "t", "t2")
    frc.notify_task_finished(ctx, "t2")
