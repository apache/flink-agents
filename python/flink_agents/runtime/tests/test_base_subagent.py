...
def test_transfer_moves_bookkeeping_onto_a_different_continuation() -> None:
    ...
    setup.on_task_prepared(from_task)
    first = setup.submit(None, "p")
    setup.pending_call_registry().track_pending_subagent_call("sid#call-1")
    setup.on_task_transferred(from_task, to_task)

    setup.on_task_prepared(to_task)
    second = setup.submit(None, "p")
    ...
    with pytest.raises(RuntimeError, match=r"act-next.*sid#call-1"):
        setup.on_task_finished(to_task)
...
def test_new_action_restarts_call_ordinal_for_a_reused_session_id() -> None:
    ...
    """
    setup = _RecordingBaseSetup()
    setup.on_task_prepared(_FakeTask(sequence_number=1))
    first = setup.submit(None, "p", "shared-session")
    second = setup.submit(None, "p", "shared-session")
    ...
    setup.on_task_finished(_FakeTask(sequence_number=1))
    setup.on_task_prepared(_FakeTask(sequence_number=2))
    reused = setup.submit(None, "p", "shared-session")
...
def test_interleaved_executions_of_one_action_keep_apart() -> None:
    ...
    setup.on_task_finished(first_task)
