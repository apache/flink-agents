...
def test_add_and_get(  # noqa:D103
    ...
    retrieved: List[MemoryItem] = memory_set.get()
...
def test_get_recent_n(  # noqa:D103
    ...
    retrieved: List[MemoryItem] = memory_set.get_recent(10)
    retrieved_msgs = [item.value for item in retrieved]
...
def test_search(  # noqa:D103
    ...
    retrieved: List[MemoryItem] = memory_set.search(query="The no.10 message", limit=1)
    retrieved_msgs = [item.value for item in retrieved]
...
def test_reduce_summarize(  # noqa:D103
    ...
    assert retrieved[0].created_at.start < retrieved[0].created_at.end
...
def test_update_metadata(  # noqa:D103
    ...
    assert item.last_accessed_at > item.created_at.start
    assert item.access_frequency == 2
