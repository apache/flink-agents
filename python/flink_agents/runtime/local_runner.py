...
class LocalRunner(AgentRunner):
    ...
    def __init__(self, agent: Agent, config: AgentConfiguration) -> None:
        ...
        from flink_agents.runtime.python_java_utils import (
            instantiate_python_event_listener,
        )
        ...
        self.__listeners = []
        listener_paths = config.get(AgentConfigOptions.EVENT_LISTENERS)
        if listener_paths:
            for path in listener_paths:
                self.__listeners.append(instantiate_python_event_listener(path))
    ...
    def run(self, **data: Dict[str, Any]) -> Any:
        ...
        **data : dict[str, Any]
        ...
        while len(context.events) > 0:
            ...
            # Trigger listeners before processing the event
            if self.__listeners:
                ...
                for listener in self.__listeners:
                    self._trigger_listener(listener, event_context, event)
    ...
    def _trigger_listener(
        self, listener: EventListener, event_context: EventContext, event: Event
    ) -> None:
        """Trigger a single listener with error handling."""
        try:
