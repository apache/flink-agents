...
class EventListenerMeta(ABCMeta):
    ...
    ``EventListener``. The resulting string format is ``module:class_path.on_event_processed``,
    which is specifically designed to be parsed by the agent's runtime (e.g., in Java
    via Pemja) to dynamically instantiate the listener class.
    ...
    """
    def __str__(cls):
        """Return a string representation of the listener class for dynamic instantiation.
...
class EventListener(ABC, metaclass=EventListenerMeta):
    ...
    EventListener provides a callback mechanism triggered at the beginning of event processing.
    ...
    **Note:** Implementing classes must provide a public no-argument constructor to
    ...
    def on_event_processed(self, context: EventContext, event: Event) -> None:
        ...
        This method is invoked when an event is received by the agent, before it is processed by
        ...
        **Important:** This method should not throw exceptions as they will be caught
