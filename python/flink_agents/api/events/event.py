...
class DynamicEvent(Event):
    """An event identified by a string identifier instead of a class type.

    Allows users to send custom events without defining a new Event subclass.
    All Extra keyword arguments are stored as dynamic attributes.
    ...
    ----------
    identifier : str
        The string identifier used for routing this event to listening actions.
    """
...
def get_event_type(event: Event) -> str:
    """Get the routing type string for an event.

    For DynamicEvent, returns the user-defined identifier.
    For all other events, returns the fully qualified class name.
    """
